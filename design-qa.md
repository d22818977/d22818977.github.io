...
- 保留 Astro / Retypeset、其余文章网址、倒计时逻辑、主题切换、RSS 与 GitHub Pages 发布方式。
...
- 桌面端 1440 × 1024：页面宽度与滚动宽度均为 1440px；横向站点栏、倒计时带、两条时间轴文章和页脚完整落在首屏，无固定侧栏残留。倒计时高度约 61px，中文标语与四组数字均为 22.4px，并保持同一基线。
- 平板端 768 × 1024：页面滚动宽度为 768px；导航右缘 656px、主题按钮左缘 685px，二者保持安全间距；倒计时高度约 98px，中文标语与四组数字均为 19.2px，数字仍为同一行。
- 手机端 390 × 844：页面滚动宽度为 390px；倒计时高度约 81px，中文标语与四组数字均为 14.72px，且标语与四个计时单元均严格单行；时间轴位于 x=96px，第一篇标题高度 24px，保持单行；所有摘要均使用单行省略。
...
5. 后续回归：压缩桌面倒计时留白并采用基线对齐；删除《我的婆婆叫黄一出》后，该网址进入 404，手机端倒计时仍为 76px 高、四组数字保持单行。
6. 更新倒计时文案为“翻过这座山，让他们听到你的故事”，目标时间调整为北京时间 2029-09-17 21:05；标语与四组数字在各断点使用相同且缩小后的字号，桌面、平板、手机端均无换行或横向溢出。
